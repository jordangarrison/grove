impl GroveApp {
    ...
    fn build_project_sidebar_lines(&self, theme: UiTheme) -> (Vec<SidebarListLine>, Option<usize>) {
    ...
    fn build_task_sidebar_lines(&self, theme: UiTheme) -> (Vec<SidebarListLine>, Option<usize>) {
        let mut lines = Vec::new();
        let mut selected_line = None;

        for (task_index, task) in self.ordered_tasks().into_iter().enumerate() {
            if task_index > 0 {
                lines.push(SidebarListLine::project(Vec::new()));
            }
            ...
            let workspace_indices = self.task_workspace_indices(task);
            if workspace_indices.is_empty() {
                lines.push(SidebarListLine::project(vec![SidebarSegment {
    ...
    fn build_sidebar_lines(&self, theme: UiTheme) -> (Vec<SidebarListLine>, Option<usize>) {
        if self.tasks.is_empty() {
            return self.build_project_sidebar_lines(theme);
        }

        self.build_task_sidebar_lines(theme)
    }
